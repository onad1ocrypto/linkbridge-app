...
export const PairingModal: React.FC<PairingModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-in fade-in select-none">
      <div className="relative w-full max-w-md bg-[#0e1017] border border-amber-500/25 rounded-3xl p-6 shadow-2xl shadow-amber-950/30 overflow-hidden text-slate-100 max-h-[90vh] overflow-y-auto">
        {/* Background glow */}
        ...
        <div className="flex flex-col items-center justify-center bg-white p-5 rounded-2xl shadow-xl my-3.5 border-2 border-amber-400/40">
          <QRCodeSVG
            value={phoneUrl}
            size={190}
            level="M"
        ...
        {/* PIN Code Box */}
        <div className="bg-slate-950/90 border border-amber-500/25 rounded-2xl p-3.5 space-y-2 mb-3.5 shadow-inner">
        ...
        {/* Custom Room PIN */}
        <form onSubmit={handleApplyRoom} className="space-y-2 mb-4">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="LINK-1234"
              className="flex-1 px-3 py-2.5 bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl text-xs text-slate-200 font-mono focus:outline-none uppercase"
            />
            <button
              type="submit"
              className="px-3.5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-semibold transition-colors border border-slate-700"
            >
              {t.apply}
            </button>
            <button
              ...
              className="p-2.5 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-amber-300 rounded-xl text-xs transition-colors border border-slate-700"
              title={t.generateNew}
            >
        ...
        <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between">
          <span className="text-xs text-slate-400">{t.testInBrowser}</span>
          <button
            onClick={() => {
              onSwitchToPhone();
              onClose();
            }}
            className="flex items-center gap-1.5 text-xs text-amber-300 hover:text-amber-200 font-semibold px-3 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/20"
          >
            <span>{t.openPhoneMode}</span>
