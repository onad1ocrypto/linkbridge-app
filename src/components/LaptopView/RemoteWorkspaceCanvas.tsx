...
export const RemoteWorkspaceCanvas: React.FC<RemoteWorkspaceCanvasProps> = ({
  ...
  const [typedText, setTypedText] = useState(
    currentLang === 'id'
      ...
      : currentLang === 'zh'
      ? '来自电脑端的问候！在手机触控板上滑动或拖动右侧滚动条。'
      : currentLang === 'vi'
      ? 'Xin chào từ Laptop! Lướt bàn di chuột điện thoại hoặc kéo thanh cuộn bên phải.'
      : 'Hello from Laptop! Slide your phone trackpad or drag the SCROLL strip on the right.'
  );
  ...
  const themes = [
    { id: 'gold', name: 'Gold Obsidian', gradient: 'from-amber-600 via-yellow-500 to-amber-700' },
    { id: 'cyan', name: 'Cyber Cyan', gradient: 'from-cyan-600 via-teal-500 to-emerald-600' },
    { id: 'purple', name: 'Royal Velvet', gradient: 'from-purple-600 via-indigo-500 to-pink-600' },
  ];

  const currentThemeObj = themes.find((th) => th.id === accentTheme) || themes[0];

  const cycleTheme = () => {
    const nextIdx = (themes.findIndex((th) => th.id === accentTheme) + 1) % themes.length;
  ...
  return (
    <div className="space-y-4">
      {/* Top Banner with Instructions and Native OS Assistant Helper Button */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl bg-gradient-to-r from-amber-500/10 via-slate-900/90 to-slate-950 border border-amber-500/25 shadow-lg">
        ...
        {/* Action Button: Open Native OS Agent Guide */}
        {onOpenDesktopAgent && (
          <button
            onClick={onOpenDesktopAgent}
            className="self-start sm:self-auto px-3.5 py-2 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 hover:text-amber-200 border border-amber-500/40 text-xs font-bold flex items-center gap-2 transition-all shadow-md shadow-amber-950/20"
          >
            <Zap className="w-4 h-4 text-amber-400" />
            <span>{t.activateOsAgent}</span>
          </button>
        )}
      </div>
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center p-6 rounded-3xl bg-slate-900/80 border border-amber-500/20 text-center space-y-3 shadow-xl">
            ...
            >
              ...
              <span>{t.leftClick} (Score: {score})</span>
            </button>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs text-slate-400 px-1">
              ...
              <span className="text-[11px] text-slate-500">{t.scroll}</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-2 hover:border-amber-500/40 transition-colors">
                <div className="flex items-center gap-2 text-amber-400 text-xs font-bold">
                  ...
                  <span>{t.connected} & Real-time</span>
                </div>
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 flex items-center justify-between shadow-md">
            ...
            <button
              onClick={() => setTypedText('LinkBridge OK: ' + new Date().toLocaleTimeString())}
